...
public class CityMaker_Test : MonoBehaviour
{
    ...
    void MakeTiles(string tiles)
    {
        ...
        for (int i=0; i<tiles.Length; i++) {
            if (tiles[i] == '>' || tiles[i] == '<') {
                ...
                position = new Vector3(x * tileSize, 0, y * tileSize);
                tile = Instantiate(zebraPrefab, position, Quaternion.identity);
                tile.transform.parent = transform;
                x += 1;
            } else if (tiles[i] == 'Z') {
                position = new Vector3(x * tileSize, 0, y * tileSize);
                tile = Instantiate(zebraPrefab, position, Quaternion.Euler(0, 90, 0));
                tile.transform.parent = transform;
                x += 1;
            } else if (tiles[i] == 'D') {
                position = new Vector3(x * tileSize, 0, y * tileSize);
